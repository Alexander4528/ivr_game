...
background_image = pygame.image.load("Sprites and objects/Objects, background and other/Background.jpeg") if dark_mode \
...
background_image = pygame.transform.scale(background_image, (1280, H))
...
def main_menu():
    ...
    while menu:
        ...
        for j, opt in enumerate(options):
            color = (255, 150, 255) if j == selected_option else (0, 0, 0)
...
def skin_menu():
    ...
    while in_skin_menu:
        ...
        # Кнопка "Назад" (выделяется при выборе)
        back_color = (255, 255, 255) if selected_skin_index == len(skins) else (0, 0, 0)
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_RETURN:
                    if selected_idx == 0:
                        ...
                        background_image = pygame.image.load("Sprites and objects/Objects, "
                                                             "background and other/Background.jpeg")\
                            if pending_mode \
                            else pygame.image.load("Sprites and objects/Objects, "
                                                   "background and other/Background_light.jpeg")
                        if background_image == pygame.image.load("Sprites and objects/Objects, "
                                                                 "background and other/Background.jpeg"):
                            background_image = pygame.transform.scale(background_image, (level_1_part_1_WIDTH, H))
                    elif selected_idx == 4:
                        cursor = saving.cursor()
...
def management_menu():
    ...
    while True:
        ...
        draw_text("Выход", font_small, (0, 0, 0), screen, W // 2, H - 50)
        pygame.display.flip()
        clock.tick(60)
...
# Выбор уровней
def level_menu():
    global levels_in, music_playing, from_menu, from_level, level1_cleared, level2_cleared, background_image, current_difficulty
    cursor = saving.cursor()
    ...
    back_option_index = len(options) - 1
    ...
    while levels_in:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_RETURN:
                    if selected_idx == back_option_index:
                        main_menu()
                        levels_in = False
                    else:
                        j = selected_idx
...
def upgrade():
    ...
    last_move_time_up = 0
    last_move_time_down = 0
    MOVE_DELAY = 500  # миллисекунды, настройте под удобство
    ...
    while upgrading:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_RETURN:
                    # Обработка выбора
                    if selected_idx == len(options):  # "Назад"
                        save_upgrades()
                        load_upgrades()
                        main_menu()
                        return
        ...
        back_color = (255, 255, 255) if selected_idx == 5 else (0, 0, 0)
        draw_text("Главное меню", font_small, back_color, screen, W // 2, H - 50)
        pygame.display.flip()
        clock.tick(60)
...
def level_1_part_1():
    ...
    level_surface.blit(background_image, (0, 0))  # Основной фон
    level = load_game_sql()
    if level != 1:
        level_1_part_1_scroll_pos = 0
    # Рисуем землю
