...
me_damaged_image = pygame.image.load('Me_damaged.png')
...
skins = [
    {'name': 'Классика',
     'unlocked': True,
     'image': 'Me.png',
     'walk_right': ['Me-right1.png', 'Me-right2.png', 'Me-right3.png', 'Me-right4.png'],
     'walk_left': ['Me-left1.png', 'Me-left2.png', 'Me-left3.png', 'Me-left4.png'],
     'damaged': 'Me_damaged.png',
     'unlock': ' ',
     'img': surface_image},
    {'name': 'Потерянный',
     'unlocked': True,
     'image': 'Lost_Me.png',
     'walk_right': ['Lost-right1.png', 'Lost-right2.png', 'Lost-right3.png', 'Lost-right4.png'],
     'walk_left': ['Lost-left1.png', 'Lost-left2.png', 'Lost-left3.png', 'Lost-left4.png'],
     'damaged': 'Lost_Me-Damaged.png',
     'unlock': ' ',
     'img': surface_image},
    {'name': 'Соник',
     'unlocked': False,
     'image': 'Sonic.png',
     'walk_right': ['Sonic-right1.png', 'Sonic-right2.png', 'Sonic-right3.png', 'Sonic-right4.png'],
     'walk_left': ['Sonic-left1.png', 'Sonic-left2.png', 'Sonic-left3.png', 'Sonic-left4.png'],
     'unlock': 'Открой способность "Бег"',
     'damaged': 'Sonic-damaged.png',
     'img': surface_image},
    {'name': 'Марио',
     'unlocked': False,
     'image': 'Mario.png',
     'walk_right': ['Mario_right1.png', 'Mario_right2.png', 'Mario_right3.png', 'Mario_right4.png'],
     'walk_left': ['Mario_left1.png', 'Mario_left2.png', 'Mario_left3.png', 'Mario_left4.png'],
     'unlock': 'Открой способность "Двойной прыжок"',
     'damaged': 'Mario_damaged.png',
     'img': surface_image},
    {'name': 'Эксклюзивный',
     'unlocked': False,
     'image': 'exclusive.png',
...
#сохранение скина
def save_skin(filename='Skin_selected.json'):
...
def save_game(play, current_score=None, filename='last.json'):
    global level_1_scroll_pos
    game_state = {
        'player_pos': level_1_scroll_pos,
...
def save_upgrades(play, filename='upgrades.json'):
    current_char_stats = {
        'player_points': player_points,
        'Attack': play.attack,
        'HP' : play.HP,
...
def load_game(play, filename='last.json'):
    global current_skin_index, level_1_scroll_pos

    try:
        with open(filename, 'r') as f:
            game_state = json.load(f)
            level_1_scroll_pos = game_state['player_pos']
...
def load_skin():
    global current_skin_index
    try:
        with open('Skin_selected.json', 'r') as f:
...
#Переключение музыки
music_on = True
menu_music = 'Carmen_Twillie_The_Lion_King_-_Circle_Of_Life_48727462.mp3'
level_1_music = 'Смешарики - Погоня.mp3'
def play_menu_music():
...
def play_level_music():
    global music_playing, music_on, playing_level
    if not music_playing and music_on and playing_level:
        try:
            pygame.mixer.music.load(level_1_music)
            pygame.mixer.music.set_volume(0.4)
            pygame.mixer.music.play(-1)
            music_playing = True
            music_on = True
...
#Пауза
def pause():
    global music_on, playing_menu, from_level, from_menu, score
    load_settings()
    options_pause_rects = [
...
class Monster:
    ...
    def spawn(self):
        ...
        if direction == 0:
            ...
            try:
                self.image = pygame.transform.scale(pygame.image.load('Enemy-left.png'), (90, 90))
            except:
                self.image = pygame.Surface((90, 90))
        else:
            self.x_speed = -self.speed
    ...
    def kill(self):
        try:
            self.image = pygame.transform.scale(pygame.image.load('EnemyDead.png'), (90, 90))
        except:
            self.image = pygame.Surface((90, 90))
        self.is_dead = True
                ...
                for idx, rect in enumerate(options_settings_rects):
                    if rect.collidepoint(mouse_pos):
                        ...
                        if selected_idx == 0:
                            ...
                            with open('last.json', 'w'):
                                pass
                            with open('upgrades.json', 'w'):
                                pass
...
def upgrade():
    ...
    pluses = []
    minuses = []
    options = ['Атака', 'HP', 'Бег', 'Двойной прыжок']
    upgrade_chars = [player.attack, player.HP, player.running_unlocked, player.double_jump_unlocked]
    upgrading = True
    for j in range(4):
    ...
    while upgrading:
        ...
        for j, opt in enumerate(options):
            ...
            if is_hovered_minuses and mouse_clicked:
                if j == 0 and player_points >= 1:
                    upgrade_chars[j] -= 1
                    player_points += 1
                elif j == 1 and player_points >= 1:
...
#Первая часть уровня
def level_1():
    global level_part_1, level_1_scroll_pos, level_1_in, menu, playing_level, playing_menu, score
    portal_rect = portal_image.get_rect(center=(LEVEL_1_WIDTH - 200, H - GROUND_H - 45))
    ...
    level_surface = pygame.Surface((LEVEL_1_WIDTH, H))
    level_surface.fill((92, 148, 252))  # Основной фон
    score, level_1_scroll_pos = load_game(player)
    # Рисуем землю
    for x in range(0, LEVEL_1_WIDTH, ground_image.get_width()):
    ...
    while level_1_in:
        ...
        screen.blit(level_surface, (0, 0), (level_1_scroll_pos, 0, W, H))

        # Отрисовываем портал (с учетом скролла)
        screen.blit(portal_image, (portal_rect.x - level_1_scroll_pos, portal_rect.y))
        ...
        if not paused:
            ...
            if player.rect.centerx > W // 2 and level_1_scroll_pos < LEVEL_1_WIDTH - W:
                # Игрок движется вправо - скроллим фон
                scroll_amount = player.rect.centerx - W // 2
                scroll_speed = 5  # Скорость скролла
                if keys[pygame.K_LSHIFT] or keys[pygame.K_RSHIFT]:
                    scroll_speed *= 2
                level_1_scroll_pos += min(scroll_amount, scroll_speed)
                # Фиксируем игрока в центре
                player.rect.centerx = W // 2
            elif player.rect.centerx < W // 2 and level_1_scroll_pos > 0:
            ...
            if player.rect.colliderect(pygame.Rect(
                    portal_rect.x - level_1_scroll_pos,
                    portal_rect.y,
                    portal_rect.width,
                    portal_rect.height
            )):
                level_1_scroll_pos = 3200
                save_game(player, score)
                level_part_1 = False
                level_2()
                return
        ...
        # Индикатор прогресса
        progress = level_1_scroll_pos / (LEVEL_1_WIDTH - W)
...
#Вторая часть уровня
def level_2():
    global monsters, last_spawn_time, spawn_delay, score, level_1_scroll_pos, player_points, HP, from_level, from_menu, \
        playing_menu
    load_upgrades(player)
    HP = player.HP
