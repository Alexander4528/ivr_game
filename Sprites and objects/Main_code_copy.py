...
def load_game(play, filename='last.json'):
    ...
    try:
        with open(filename, 'r') as f:
            ...
            if 'current_skin_index' in game_state:
                current_skin_index = game_state['current_skin_index']
                apply_skin(current_skin_index)

            if hasattr(play, 'apply_current_skin'):
                play.apply_current_skin()
...
#Главное меню
def main_menu():
    global player, menu, levels_in, from_menu, from_level
    load_settings()
    menu = True
    options_rects = [
...
#Вторая часть уровня
def level_2():
    global monsters, last_spawn_time, spawn_delay, score, level_1_scroll_pos, player_points, HP
    load_upgrades(player)
    HP = player.HP
